...
export default async (pwa: PWAContext) => {
  ...
  if (nuxt.options.dev) { consola.warn('[PWA] Workbox is running in development mode') }

  // Use Workbox CDN by default
  if (!options.workboxUrl) { options.workboxUrl = `https://storage.googleapis.com/workbox-cdn/releases/${options.workboxVersion}/workbox-sw.js` }

  if (!options.cacheOptions.revision) { options.cacheOptions.revision = randomString(12) }
  ...
  if (options.autoRegister) { addNitroPlugin(nuxt, pwa._resolver.resolve('./runtime/nitro/register-plugin')) }
}

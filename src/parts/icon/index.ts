...
export default async (pwa: PWAContext) => {
  ...
  if (!existsSync(options.source)) { return consola.warn(`[PWA] Icon not found at ${options.source}`) }
  ...
  if (options.sizes.length === 0) { options.sizes = [64, 120, 144, 152, 192, 384, 512] }
  ...
  if (isSplashSupportEnabled) {
    if (!options.splash.backgroundColor) { options.splash.backgroundColor = pwa.manifest.background_color }

    if (options.splash.devices.length === 0) { options.splash.devices = defaultDevices }
